...
void GlobalFun::computeRandomwalkNeighborhood(CMesh *mesh, int one_ring_knn, int max_knn)
{
  ...
  computeAnnNeigbhors(mesh->vert, mesh->vert, one_ring_knn, false, "computeRandomwalkNeighborhood");
  ...
  for (int i = 0; i < mesh->vert.size(); i++)
  {
    ...
    for (int j = 0; j < v.neighbors.size(); j++)
    {
      mesh->vert[(*neighbors)[j]].is_skel_virtual = true;
    ...
    while (!neighbors_queue.empty())
    {
      ...
      CVertex& t = mesh->vert[neighbor_index];

      for (int k = 0; k < t.neighbors.size(); k++)
      {
        if (mesh->vert[t.neighbors[k]].is_skel_virtual)
        {
          continue;
        }
