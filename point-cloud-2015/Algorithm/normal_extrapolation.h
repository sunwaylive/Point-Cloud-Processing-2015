	...
	*/
	...
	private:
			...
			**************************************************/
			...
			{
				RiemannianEdge(Plane *p=NULL, ScalarType w=1000000000. /* = std::numeric_limits<ScalarType>::max()*/)  {plane=p; weight=w; }
			...
			struct MSTEdge
			{
				MSTEdge(Plane *p0=NULL, Plane *p1=NULL, ScalarType w=1000000000./*=std::numeric_limits<ScalarType>::max()*/) {u=p0; v=p1; weight=w;};
    ...
    static void ExtrapolateNormalsWithExistingNeighbor(const VertexIterator &begin, const VertexIterator &end, NormalOrientation orientation=IsCorrect, CallBackPos *callback=NULL)
    {
      BoundingBoxType dataset_bb;
      for (VertexIterator iter=begin; iter!=end; iter++)
        dataset_bb.Add(iter->P());
      ...
      for (VertexIterator iter=begin; iter!=end; iter++)
      {
        ...
        // for each vertex *iter, compute the centroid as avarege of the k-nearest vertices of *iter
        Plane *plane = &tangent_planes[ std::distance(begin, iter) ];
        for (unsigned int n=0; n<iter->neighbors.size(); n++)
          plane->center += nearest_points[n];
        plane->center /= ScalarType(iter->neighbors.size());
      ...
      for (PlaneIterator iPlane=tangent_planes.begin(); iPlane!=ePlane; iPlane++)
      {
        ...
        unsigned int kk = iter->neighbors.size();
        PlanePointDistanceFunctor ppdf;
        DummyObjectMarker dom;
        octree_for_plane.GetKClosest
          (ppdf, dom, kk, iPlane->center, max_distance, nearest_planes, distances, nearest_points, true, false);
